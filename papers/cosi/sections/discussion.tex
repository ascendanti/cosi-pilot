...
The pilot does not show that the alignment is robust to held-out
prompts. Procrustes was fit and evaluated on the same probe set; no
train/test split was performed (Study C).

The pilot does not show that the alignment exceeds simple lexical
features (Study B), or that the promised CKA cross-validation
\cite{kornblith2019similarity} agrees with the Procrustes verdict
(Study F: CKA was committed in \S\ref{sec:methods} but not computed in
this paper; this is a methodological commitment the pilot does not
satisfy).
...
\subsection{Phase~2: Training-Distribution Control (the Binding Confound)}

Among the seven studies of \S\ref{sec:requiredwork}, Study D
(training-distribution-disjoint control) is the binding open work in
the sense that no other study, however positive, can substitute for
it. The pilot is run on two model pairs whose training distributions
overlap substantially; until that overlap is broken in a controlled
way, the pilot result is consistent with shared-corpus residue and
the operator-theoretic interpretation cannot be preferred. Candidate
controls include:

\subsection{Phase~2: Training-Distribution Control}

Phase~2 addresses the most important confound: the possibility that the
aligned subspace is a residue of shared training distribution rather than
a substrate-independent computational structure. The control requires a
model whose training distribution is substantially disjoint from the
Phase~1 pair's. Candidate controls include:
