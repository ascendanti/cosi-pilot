...
catalogued, four follow-up control studies (A, B/B2/B3, C, F-CKA) run
and reported, and the remaining program (D and E) specified.
The methodologically decisive controls --- Study B1, Study B2, and the
cross-validated Study B3 --- show that the pilot's headline numbers
are dominated by shared lexical and template encoding rather than by
substrate-shared computation. The cross-validated CKA collapse from
$0.84$--$0.96$ to $0.35$--$0.59$ on held-out residuals is the honest
version of what the original Procrustes-against-row-permutation-null
test was actually measuring. The most defensible publishable claim
of the present paper is therefore methodological: $z$-scores against
row-permutation nulls in cross-model representation tests do not
licence operator-theoretic, mechanistic, or compositional-reasoning
interpretations unless lexical structure is explicitly controlled
and the controls are themselves cross-validated. The operator-theoretic
COSI program remains a motivated research agenda; the present pilot
does not advance its strong claim. The next decisive step is Study D
(training-distribution-disjoint control), which alone could
discriminate whether the surviving residualized cross-model signal
is substrate-independent or shared-corpus residue.
